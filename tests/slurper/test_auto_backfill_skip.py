...
def _run_auto_backfill(
    ...
    *,
    ...
    supervisor: RecordingSupervisor | None = None,
...
def test_auto_backfill_declares_phase_sequence(
    ...
    assert sleep_calls == [30, 60.0]
    phases = [(item.task_name, item.phase, item.details) for item in supervisor.declarations]
    assert ("auto-backfill", "startup_sleep", None) in phases
    assert ("auto-backfill", "listing_channels", None) in phases
    assert ("auto-backfill", "checking_skip", {"channel_id": "C_ONE"}) in phases
    assert ("auto-backfill", "channel", {"channel_id": "C_ONE"}) in phases
    assert ("auto-backfill", "inter_channel_sleep", {"channel_id": "C_TWO"}) in phases
    assert ("auto-backfill", "channel", {"channel_id": "C_TWO"}) in phases
    assert ("auto-backfill", "complete", None) in phases
