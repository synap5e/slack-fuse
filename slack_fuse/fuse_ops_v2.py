...
#: Always-present root ghost that keeps recursive tree walkers away from
#: operator-only surfaces. The inode sits immediately below NO_POSTGRES's
#: reserved inode, well above the persistent allocator's practical range.
IGNORE_FILE_NAME: Final = ".ignore"
IGNORE_FILE_INODE: Final = (1 << 53) - 2
IGNORE_FILE_CONTENT: Final = b"""\
# Auto-generated by slack-fuse. Skips operator-only surfaces during
# tree walks. Delete/replace at your own risk.
_control/
NO_POSTGRES
"""
...
class SlackFuseOpsV2(pyfuse3.Operations):
    ...
    async def getattr(
        ...
        with self._callback_guard("getattr", inode=inode):
            # Reserved root ghosts are handled entirely without DB access.
            if inode in {IGNORE_FILE_INODE, NO_POSTGRES_INODE}:
                return self._reserved_root_attr_or_enoent(inode)
    ...
    def _reserved_root_attr_or_enoent(self, inode: int) -> pyfuse3.EntryAttributes:
        if inode == IGNORE_FILE_INODE:
            set_path(f"/{IGNORE_FILE_NAME}")
            return _make_file_attr(
                IGNORE_FILE_INODE,
                len(IGNORE_FILE_CONTENT),
                timeout_s=_IMMUTABLE_FILE_TIMEOUT_S,
            )
    ...
    async def lookup(
        ...
        with self._callback_guard("lookup", inode=parent_inode):
            ...
            if parent_inode == ROOT_INODE and child_name == IGNORE_FILE_NAME:
                set_path(f"/{IGNORE_FILE_NAME}")
                return _make_file_attr(
                    IGNORE_FILE_INODE,
                    len(IGNORE_FILE_CONTENT),
    ...
    def _snapshot_dir(self, path: str) -> list[tuple[str, pyfuse3.EntryAttributes, int]]:
        ...
        if path == "/":
            # Reserved root ghosts follow normal entries so their token IDs do
            # not shift the stable directory surface between mount lifetimes.
            entries.append((IGNORE_FILE_NAME, False))
            if self._pg_health is not None and self._pg_health.is_down():
                entries.append((NO_POSTGRES_NAME, False))
        for idx, (name, is_dir) in enumerate(entries):
            child_path = f"/{name}" if path == "/" else f"{path}/{name}"
            if child_path == f"/{IGNORE_FILE_NAME}":
    ...
    async def read(self, fh: int, off: int, size: int) -> bytes:
        with self._callback_guard("read", inode=fh):
            if fh == IGNORE_FILE_INODE:
                set_path(f"/{IGNORE_FILE_NAME}")
                return IGNORE_FILE_CONTENT[off : off + size]
