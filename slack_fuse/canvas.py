...
def fetch_canvas_markdown(
    ...
    data = resp.json()
    if not data.get("ok"):
        log.warning("files.info failed for %s: %s", file_id, data.get("error"))
        return None

    url = data["file"].get("url_private", "")
    if not url:
        return None
    ...
    if page_resp.status_code != 200:
        log.warning("Canvas download failed for %s: HTTP %d", file_id, page_resp.status_code)
        return None
