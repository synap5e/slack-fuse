...
The shim reaches medina over mTLS using an exported copy of medina's `apps/slack-fuse-client-tls`. The PEMs live in the k8s-homelab SOPS secret `apps/slack-fuse-env` (keys `NATS_CA_CRT` / `NATS_TLS_CRT` / `NATS_TLS_KEY`), mounted read-only at `/etc/nats/` with `defaultMode: 0440` (server runs as uid=10001 gid=0; 0400 is unreadable). Rotation is manual — the copy does not follow the source — and is triggered by an alert on the medina side (`~/agentic/gcp-k8s/docs/runbooks.md`). **No slack-fuse health signal observes this credential's expiry.** `/health`, `/livez`, `slurper-health` and the client trailer all stay green while the shim cannot connect; the only path failure is `nats_shim.iteration_restart` spans in the slurper log.

**Production ingest is single-tracked on NATS today.** `SLACK_FUSE_SERVER_NATS_SHIM_ENABLED=true`, `SLACK_FUSE_SERVER_WEBHOOK_PORT=18766`, `SLACK_FUSE_SERVER_SOCKET_MODE_ENABLED=false`. All three would suggest 2-of-3 or at least webhook-as-fallback, but the measured truth as of 2026-09-22 is that `slack_event_inbox` has ZERO webhook-transport rows all-time — every one of its 37k rows since 2026-09-07 (the day the shim went live) is `source_transport='nats'`. The webhook listener is bound and healthy but decorative: Slack's Events API request URL points at medina, which forwards to NATS, so nothing calls port 18766 directly. Restoring webhook-as-real-backup would require Atlas to re-fork Slack's traffic — out of our unilateral control.

**Consequences to plan around.** If the NATS shim dies (dead cert, medina outage, ACL change, network partition to GCP), ingest goes to **total silence**, not half-death. No health signal covers this: `/health`, `/livez`, `slurper-health` and the client trailer stay green while zero events land. Ground truth is `SELECT source_transport, count(*) FROM slack_event_inbox WHERE received_at > NOW() - INTERVAL '5 minutes' GROUP BY 1;` — configured surface up top, actual receipt on the ground; the two answered different questions on 2026-08-31 → 2026-09-07 when Atlas repointed Slack's endpoint and our webhook stayed 1/1 receiving zero for 6 days.

Chained with the credential above: the mTLS cert is a single point of **total** ingest failure, not a degraded-mode risk. The copy we hold expires 2027-09-22 (re-exported 2026-09-23). Renewal is disabled on the medina source, so nothing will ever rotate it for us; medina's `ExportedCertificateExpiringSoon` alert fires 30 days out, and the re-export procedure is in `~/agentic/gcp-k8s/docs/runbooks.md`. A future reader who plans to lean on webhook-as-backup will be leaning on nothing.

**The detection that would cover every cause of silence is not built yet.** A Loki absence alert on `op=nats_shim.enqueue … status=ok` over 90 minutes (no false positives in 16 days of history; the longest non-outage gap was 52 minutes) is written up in the k8s-homelab repo's `BACKLOG.md`. It is deferred because that cluster has no push channel yet, and routing it to Slack would fail by the same mechanism it reports. Until it exists, an ingest stop is found by someone noticing, as it was on 2026-08-31.
