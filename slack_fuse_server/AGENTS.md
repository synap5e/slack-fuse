...
Chained with the credential above: the mTLS cert is a single point of **total** ingest failure, not a degraded-mode risk. It expires 2027-08-29 on our side, auto-renews on the source at an interval we don't observe, and had no working alert coverage on the platform side (Atlas is fixing that separately). The observer cert on the same rotation cycle lands in 39 days. A future reader who plans to lean on webhook-as-backup will be leaning on nothing.
