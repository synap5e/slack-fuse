...
Production runs exactly two ingest transports today — Events API webhook (`SLACK_FUSE_SERVER_WEBHOOK_PORT=18766`) and the NATS shim; Socket Mode is disabled (`SLACK_FUSE_SERVER_SOCKET_MODE_ENABLED=false`). If the shim dies, the webhook is the ONLY surviving path — not "one of two other transports". And "surviving" here means "still configured to listen", not "still receiving": we lived through 2026-08-31 → 2026-09-07 when Atlas repointed Slack's Events API away and our webhook listener stayed 1/1 for 6 days receiving zero. **No health signal distinguishes "webhook configured" from "webhook receiving traffic".** The only way to answer "is ingest actually happening" is `SELECT source_transport, count(*) FROM slack_event_inbox WHERE received_at > NOW() - INTERVAL '5 minutes' GROUP BY 1;` — configured surface up top, actual receipt on the ground.
