...
- Slack producer constraints not implemented: emit one synthetic root parenting `channels`/`dms`/`group-dms`/`other-channels`/`_workspace`; enforce 255-byte UTF-8 `Name::Title`; emoji/variation-selector normalization. Current `derive_thread_slug` can produce oversized names — latent `Name::Final` Nack. [6260]–[6266]
  - **DISPROVEN 2026-09-18 against the tree.** Every filesystem name goes through `slack_fuse/slug.py::slugify`, which NFKD-normalizes, `.encode("ascii", "ignore")`s away all non-ASCII, maps `[^a-z0-9]+`→`-`, and hard-caps at 60 chars on a word boundary. Collision suffixes (`-2`, `-3`) add ≤6 bytes even at 10,000-way collision; fallbacks are `ts-<message_ts>` and `channel_id[:12]`; `derive_thread_slug` slugifies `body[:120]`. Oversized names are unreachable by construction, and the emoji/variation-selector concern is moot because non-ASCII is deleted, not normalized. The real property is the inverse: slugify is **lossy** — `#日本語` slugifies to empty and falls back to `channel_id[:12]` — so adopting a Unicode-preserving engine is a path-visible migration change (a parity/allowlist item), not a hazard fix. Caught before it was written into illus's engine requirements on our attribution.
