...
| 1F wire binary + same-port dispatch | claude (opus) | `sprint1f-wire` | `synap5e/feat/sprint1f-wire-binary` | in flight. Discovered pre-existing severe bug: 1B's `wire/tail.py` uses `psycopg.AsyncConnection` (asyncio) which raises under trio. Every WS connect crashes with real DB; the test suite masks it by skipping without DATABASE_URL. Owner sanctioned extending 1F's file ownership to include `wire/tail.py` for the trio-native rewrite (sync psycopg + to_thread + notifies-poller thread + memory channel bridge). |
