...
| `slack_fuse/api.py` | `SlackClient`: synchronous httpx wrapper around Slack REST. Generic typed `_get(method, params, response_type)` validates every response into a Pydantic model — no `Any` leaks out. Methods return typed values: `list_conversations -> list[Channel]`, `get_history -> list[Message]`, `get_replies -> Thread`, `get_file_info -> SlackFile \| None`, `search_huddle_canvases -> list[SearchFile]`, `get_history_page -> ConversationsHistoryResponse`. Exception hierarchy: `SlackAPIError` base, with `RateLimitedError` (429, has `retry_after`) and `FatalAPIError` (401/403 + body errors like `token_revoked`). 0.1s delay between paginated requests. |
...
| `slack_fuse/store.py` | The brain. `SlackStore` owns: in-memory caches (channel list, huddle index, day messages, thread slugs, threads), TTL bookkeeping, exponential backoff (`_BackoffState`), and the `cached_only_mode()` context manager. `_api_call(fn: Callable[[], T]) -> T \| None` is a typed wrapper that records backoff on failure — pass it a method reference or a lambda, never a string. Fuse ops call into `list_channels`, `get_channel_by_slug`, `get_known_dates`, `get_thread_slugs`, `get_day_messages`, `get_huddles_for_thread`, `get_huddle_index` (returns `list[HuddleIndexEntry]`, not dicts), `get_huddle_by_canvas_id`, `find_huddle_index_entry_by_canvas`, `merge_known_dates`, `force_refresh`, plus `render_*` methods that delegate to `renderer.py`. Disk-cache round-trip uses `Message.model_validate` / `model_dump(mode="json")` — no hand-rolled parsers. TTLs: `_CHANNEL_LIST_TTL=1800`, `_HUDDLE_INDEX_TTL=1800`. Day-message TTL is dynamic in `_date_ttl` (today (system local date) → `_RECENT_MSG_TTL=300`, any earlier local date → `_OLD_MSG_TTL=inf`). Thread TTL is dynamic in `_thread_ttl` based on the parent message's local date — same today-vs-not-today policy, so once a thread from a previous local day is in memory or on disk it's never re-fetched. Local-time boundary is deliberate — UTC midnight would land mid-PST-workday and lock messages while they were still being edited. |
| `slack_fuse/disk_cache.py` | Pure functions over `~/.cache/slack-fuse/`. JSON files keyed by channel/date/thread_ts/canvas_file_id. Signatures use `JsonObject` (recursive JSON type from `models`) — typed validation happens in `store.py` where the relevant Pydantic model is known. Old day messages (>7 days) are effectively immutable so they're cached indefinitely. |
| `slack_fuse/backfill.py` | `backfill_all`: trio task that paginates every member channel's full history into the disk cache. Two phases per channel: (1) day backfill via `conversations.history` with 30-180s sleeps and `<channel_id>.done` markers; (2) thread backfill — `_collect_thread_parents` walks the cached day messages, finds `reply_count > 0` parents, and `client.get_replies` each one with shorter 2-8s sleeps and `<channel_id>.threads.done` markers. Both phases are idempotent and resumable; existing day-only-backfilled channels get re-walked for threads only. Skips channels matching `notification`/`alert`/`prod-alerts`. Gated by `SLACK_FUSE_BACKFILL` (default false — opt in). |
