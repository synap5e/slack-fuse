...
| File | Responsibility |
|---|---|
| `slack_fuse/__main__.py` | CLI entrypoint. `mount`, `unmount`, `resolve`, and `permalink` subcommands, env-var parsing (`SLACK_FUSE_BACKFILL`), trio nursery wiring (`pyfuse3.main`, periodic channel-list refresh, archive, optional backfill). Creates the shared `CapacityLimiter(1)` that serializes all sync store/API work. Cancels the nursery scope on unmount so background tasks exit cleanly. Auto-cleans stale mounts on startup with `fusermount3 -uz`. |
| `slack_fuse/channel_stats.py` | Pure markdown renderer for the sorted `_workspace/channels.md` inventory. Formats aggregate totals, refresh time, ingest status, membership, creation date, and approximate Slack totals. |
| `slack_fuse/projector/channel_stats_fetch.py` / `channel_stats_warmer.py` | Typed authenticated `GET /channel-stats` client and five-minute background warmer. The warmer renders and populates the in-process FUSE cache; callbacks only read cached bytes and never perform HTTP. |
| `slack_fuse/fuse_ops_v2.py` | Local-PG FUSE adapter and `_control`/`_workspace` virtual-file dispatcher. `_workspace/channels.md` is read-only and cache-only on the callback path. |
| `slack_fuse/projector/disk_projection.py` / `projection_ledger.py` | Disposable atomic markdown mirror plus its PostgreSQL validity ledger. Ledger generations are authoritative for reader admission and render completion; `_dirty` / `_inflight` remain process-local scheduling caches only. |
| `slack_fuse_server/search_messages.py` | Typed user-token-only `search.messages` wrapper. Queries one named channel and flags inconsistent 10K+ paging totals as approximate. DMs are rejected before this layer by the sweep. |
| `slack_fuse_server/slurper/channel_totals.py` | Six-hour Tier-2-paced sweep over every channel-list channel. Upserts query-derived counts into `channel_message_totals`; failures preserve the prior total while updating refresh status. Also backs `slack-fuse-server refresh-channel-totals`. |
| `slack_fuse_server/channel_stats.py` | One-query server projection joining folded channel metadata, `channel_message_totals`, blocks, and ingested message-event counts for authenticated `GET /channel-stats`. |
| `slack_fuse/auth.py` | `load_tokens()` — reads `SLACK_USER_TOKEN`/`SLACK_APP_TOKEN`/`SLACK_WORKSPACE_URL` from env, falls back to `.env` and `~/.config/slack-fuse/config.json`. |
| `slack_fuse/api.py` | `SlackClient`: synchronous httpx wrapper around Slack REST with a persistent `httpx.Client` for connection pooling. Generic typed `_get(method, params, response_type)` validates every response into a Pydantic model — no `Any` leaks out. Methods return typed values: `list_conversations -> list[Channel]`, `get_history -> list[Message]`, `get_replies -> Thread`, `get_permalink -> str`, `get_file_info -> SlackFile \| None`, `search_huddle_canvases -> list[SearchFile]`, `get_history_page -> ConversationsHistoryResponse`. Exception hierarchy: `SlackAPIError` base (raised on all `ok=false`), with `RateLimitedError` (429, has `retry_after`) and `FatalAPIError` (401/403 + body errors like `token_revoked`). Exposes `.http` property so canvas/transcript/user_cache share the client. 0.1s delay between paginated requests. |
...
| `slack_fuse/fuse_ops.py` | `SlackFuseOps(pyfuse3.Operations)`. Path-string-driven dispatch via `_list_dir_impl`, `_resolve_content_impl`, `_is_dir_impl`. All FUSE callbacks delegate sync work to a worker thread via `trio.to_thread.run_sync` with a shared `CapacityLimiter(1)` so the event loop stays responsive. Handles the `.cached-only/` prefix via `_strip_cached_prefix` + `cached_only_mode()`. Generates an `index` symlink inside in-thread huddle dirs that points back to `/huddles/<month>/<day>/<slug>`. Exposes `inodes` (read-only `InodeMap`) so `__main__` can construct `InodeInvalidator` — the `InvalidationSink` implementation that translates store events to `pyfuse3.invalidate_inode` calls on the matching feed.md/channel.md/thread.md/dir inodes. |
...
| `slack_fuse/_slug_helpers.py` | Legacy shared helpers for `slack_fuse/permalink.py` only: v1 channel slug replay via `SlackClient`, day-message disk/API loading, thread slug map construction. `slack-fuse resolve` migrated to v2 (2026-08-02, commit `057883c`); `permalink` still holds the entire v1 island (`store.py`, `api.py`, `user_cache.py`, `disk_cache.py`, `renderer.py`, `fuse_ops.py`, `backfill.py`, `archive.py`, `socket_mode.py`) alive until it too is migrated. |
| `slack_fuse/resolve.py` | Forward resolver for `slack-fuse resolve`: parses Slack message permalinks and returns the matching FUSE path via the local v2 projections store. Composes `fuse_v2_helpers.assign_conv_root_slugs` / `fetch_day_thread_parents` / `dedup_thread_slug_map`. Fully local: no Slack API, no disk cache. |
| `slack_fuse/permalink.py` | Reverse resolver for `slack-fuse permalink`: parses FUSE paths, reads rendered markdown frontmatter (`channel_id`, `thread_ts`), falls back to slug replay for older archived renders, and calls `chat.getPermalink` for message URLs. Channel-root URLs require `SLACK_WORKSPACE_URL`. Still on v1 — migrating this unblocks v1-island deletion. |
...
- `SLACK_WORKSPACE_URL` is only needed for channel-root reverse permalinks (`slack-fuse permalink .../channel.md` at the channel root). Message-level reverse permalinks use Slack's `chat.getPermalink`.
...
A Plan-9-style ctl/status surface at the mount root, wired in `cmd_mount` (`SlackFuseOpsV2`). `slack_fuse/control.py` holds the thread-safe in-memory `ControlState` (last refresh/block/unblock/backfill/rerender outcome) + the HTTP-status→verb map (`result_for_status`: 202→`queued`, 409→`busy`, 401/403→`unauthorised`, 503/`0`→`server_unavailable`, else `http_<code>`). `slack_fuse/projector/refresh_fetch.py` and `slack_fuse/projector/block_fetch.py` are sync httpx helpers (mirroring `gaps_fetch.py`), pointed at the server's mutating endpoints with sub-second timeouts and `0` sentinel on transport error.
...
- Don't re-add the `ro` mount option to `cmd_mount` (it's intentionally only `{"fsname=slack-fuse"}`). `ro` makes the kernel reject every write before it reaches the daemon, killing the `_control/` triggers. Read-only is enforced in-daemon instead: `SlackFuseOpsV2.open` returns `EROFS` for any write-mode open outside `_control` writable files.
