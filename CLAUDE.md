...
| `slack_fuse/_slug_helpers.py` | Internal shared slug/cache helpers for permalink/path resolution: channel slug replay, channel lookup by id or slug, day-message disk/API loading, thread slug map construction. |
| `slack_fuse/resolve.py` | Forward resolver for `slack-fuse resolve`: parses Slack message permalinks and returns the matching FUSE path, using `_slug_helpers.py` for channel/thread slug replay and day cache warming. |
| `slack_fuse/permalink.py` | Reverse resolver for `slack-fuse permalink`: parses FUSE paths, reads rendered markdown frontmatter (`channel_id`, `thread_ts`), falls back to slug replay for older archived renders, and calls `chat.getPermalink` for message URLs. Channel-root URLs require `SLACK_WORKSPACE_URL`. |
