...
| `slack_fuse/backfill.py` | `backfill_all`: trio task that paginates every member channel's full history into the disk cache. Long random sleeps (30–180s), per-channel `.done` markers in `~/.cache/slack-fuse/backfill/`, skips channels matching `notification`/`alert`/`prod-alerts`. Gated by `SLACK_FUSE_BACKFILL` (default false — opt in). |
