...
- **Feed direction (as of 2026-09-18).** The six-seam Rust co-design is superseded. fuse-rust/vfsd v1 is terminal (maintenance, no new producers, per their ADR-0029), and its successor `illus` (`~/agentic/illus`) is built on notion-fuse's projections-over-NATS v3 contract — a NATS lane of per-unit records, an HTTP index for repair, content by hash. slack-fuse is a **co-signer on the spec crate and the bridge-facing DB contract**, consulted at spec-crate review, which is a hard gate (the spec does not freeze without it). Our feed sketch is cited as one of two independent genericity proofs. Nothing is built; our cutover is behind platform ingest gating. Correspondence in `docs/outbound/`; open items in `BACKLOG.md`.
- Under that contract the mount's own transport changes but its ingestion does not: Slack webhooks already land on the platform's JetStream estate, and the custom WS feed in `wire/` is what illus would replace. `slack_fuse/projector/` is the part with the most to lose — `projection_targets` is the acknowledged predecessor of the contract's `unit_latest` seam.
